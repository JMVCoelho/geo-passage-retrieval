...
def generate_eval_samples_full_corpus():
  eval_queries = {} #qid => query
  eval_corpus = {} #pid => passage
  eval_rel_docs = {} #qid => Set[cid]
  ...
  for qId in val_queries:
    ...
    eval_queries[qId] = all_queries[qId]
    eval_rel_docs[qId] = []


    top25 = df.query('qId == @qId')['pId'].values 
    for pId in top25:
      eval_corpus[pId] = corpus[pId]
      if isRelevant(pId, qId):
        eval_rel_docs[qId].append(pId)

    
    eval_rel_docs[qId] = set(eval_rel_docs[qId])
  
  return eval_queries, eval_corpus, eval_rel_docs

class MySentenceTransformer(nn.Sequential):
    """
    ...
    def start_multi_process_pool(self, target_devices: List[str] = None):
        ...
        for cuda_id in target_devices:
            p = ctx.Process(target=MySentenceTransformer._encode_multi_process_worker, args=(cuda_id, self, input_queue, output_queue), daemon=True)
            p.start()
            processes.append(p)
    ...
    def fit(self,
        ...
        if output_path_best_val is not None:
            os.makedirs(output_path_best_val, exist_ok=True)
        ...
        num_train_steps = int(steps_per_epoch // gradient_accumulation_steps * epochs)
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            for _ in trange(steps_per_epoch, desc="Iteration", smoothing=0.05, disable=not show_progress_bar):
                ...
                if evaluation_steps > 0 and training_steps % evaluation_steps == 0:
                    self._eval_during_training(evaluator, output_path_best_val, save_best_model, epoch,
            ...
            self._eval_during_training(evaluator, output_path_best_val, save_best_model, epoch, -1, callback)
